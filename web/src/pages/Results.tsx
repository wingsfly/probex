...
const FIELD_DICT: Record<string, [string, string]> = {
  ...
  'click_to_vd_open_ms': ['Click→Open', '点击按钮到voiceDictation WS open事件(ms)。计算: tVdOpen - tClick = 纯建连握手(TCP+TLS+WS Upgrade)。约占Click→VD的绝大部分'],
  'click_to_vd_ready_ms': ['Click→VD', '点击按钮到VD会话就绪(ms)。计算: tVdReady - tClick = WS上发出首帧的时刻。比Click→Open多算了open后到首帧的应用初始化(~几ms)'],
